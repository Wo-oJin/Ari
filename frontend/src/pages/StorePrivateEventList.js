...
const StorePrivateEventList = () => {
    ...
    return (
        <>
            <Header text="개인 이벤트 등록" link="/mypageOwner"></Header>
            <div className="container">
                <p style={{width: "327px", margin: "32px 0", fontSize: "19px"}}>개인 이벤트 목록&#40;{privateEvents.length}&#41;</p>
                {/* 개인 이벤트 내용이 담긴 문자열 배열 받아서 map 함수로 리스트 구성 */}
                {privateEvents.map((privateEvent, index) => {
                    return (
                        <div key={index}>
                            <Link to='/storeEditPrivateEvent' state={{ index: index, info: privateEvents[index] }}>
